...
router.post('/graph', function(req, res){
    ...
    var query = conn.query(`SELECT subject,score,grade from score`, function(err, result) {
        ...
        var korean = ( result.find(it => it.subject == "korean") )
        var math = ( result.find(it => it.subject == "math"))
        var english = ( result.find(it => it.subject == "english"))
        var history =( result.find(it => it.subject == "history"))
        var tamgu1 = ( result.find(it => it.subject == "tamgu1"))
        var tamgu2 = ( result.find(it => it.subject == "tamgu2"))
